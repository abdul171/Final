...
public class Sync extends AsyncTask<String,Void,String> {
    ...
    protected String doInBackground(String ... params) {
        if(params.length>0){
            if(params[0].equalsIgnoreCase("sync")){
                try {
                    onSyn(params);
    ...
    private String onInsert(String ... params){
        String phpPageULR="http://192.168.43.252:80/sqli/mysql_write.php";
        try {
    ...
    private String onDelete(String ... params){
        String phpPageULR="http://192.168.43.253:80/sqli/mysql_write.php";
        try {
            // preparing the URL for the connection
            URL url=new URL(phpPageULR);
            // open a channel between the client(android device) and the server (PHP)
            ...
            channel.setRequestMethod("POST");
            channel.setDoOutput(true);
            // create a sub-channel inside the channel to specify whether you want to write or read
            // output means write, input means read
            OutputStream subChannel=channel.getOutputStream();
    ...
    private String Read(String ... params){
        String phpPageULR="http://192.168.43.253:80/sqli/mysql_read.php";
        try {
            // preparing the URL for the connection
            URL url=new URL(phpPageULR);
            // open a channel between the client(android device) and the server (PHP)
